export default function TrainingArtifacts() {
  return (
    <div>
      <div className="page-header">
        <div className="page-title">Training Artifacts</div>
        <div className="page-desc">
          Veritas separates verdict classification from explanation generation.
          The production verifier is unchanged. All metrics below are measured.
        </div>
      </div>

      <div className="section">
        <div className="section-label">Production Status</div>
        <div className="status-grid">
          <div className="status-card">
            <div className="status-dot-sm ok" />
            <div>
              <div className="status-card-name">Production verifier</div>
              <div className="status-card-sub">transformer_verifier_clean</div>
            </div>
          </div>
          <div className="status-card">
            <div className="status-dot-sm warn" />
            <div>
              <div className="status-card-name">Relevance gate</div>
              <div className="status-card-sub">disabled by default</div>
            </div>
          </div>
          <div className="status-card">
            <div className="status-dot-sm warn" />
            <div>
              <div className="status-card-name">MLX LoRA explanation</div>
              <div className="status-card-sub">trained; partial format compliance</div>
            </div>
          </div>
          <div className="status-card">
            <div className="status-dot-sm err" />
            <div>
              <div className="status-card-name">Phi-3 QLoRA / DPO</div>
              <div className="status-card-sub">CUDA path only; not trained locally</div>
            </div>
          </div>
        </div>
      </div>

      <div className="section">
        <div className="section-label">Artifact Status</div>
        <div className="table-wrap">
          <table>
            <thead>
              <tr>
                <th>Artifact</th>
                <th>Status</th>
                <th>Notes</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td>Production verifier</td>
                <td><span className="badge badge-green">Production</span></td>
                <td>DistilRoBERTa NLI · oracle macro-F1 0.6728</td>
              </tr>
              <tr>
                <td>SFT explanation dataset</td>
                <td><span className="badge badge-blue">Built</span></td>
                <td>256 training / val / test examples</td>
              </tr>
              <tr>
                <td>DPO preference dataset</td>
                <td><span className="badge badge-blue">Built</span></td>
                <td>Synthetic rejection pairs</td>
              </tr>
              <tr>
                <td>MLX LoRA — verdict prediction</td>
                <td><span className="badge badge-green">Trained</span></td>
                <td>100 iters · acc 0.695 · macro-F1 0.4632 · 53.7 tok/s</td>
              </tr>
              <tr>
                <td>MLX LoRA — explanation SFT</td>
                <td><span className="badge badge-green">Trained</span></td>
                <td>500 iters · citation_presence 0.72 · partial format compliance</td>
              </tr>
              <tr>
                <td>Phi-3 QLoRA adapter</td>
                <td><span className="badge badge-amber">CUDA blocked</span></td>
                <td>Script + config + Colab notebook ready; no CUDA on Mac</td>
              </tr>
              <tr>
                <td>Phi-3 DPO adapter</td>
                <td><span className="badge badge-amber">CUDA blocked</span></td>
                <td>Requires QLoRA adapter first; CUDA unavailable</td>
              </tr>
              <tr>
                <td>Robust verifier retrain</td>
                <td><span className="badge badge-red">Negative result</span></td>
                <td>Regressed: retrieved F1 0.3887→0.2829; not in production</td>
              </tr>
              <tr>
                <td>Relevance gate</td>
                <td><span className="badge badge-red">Negative result</span></td>
                <td>NEI FPR 0.71→0.29 but macro-F1 0.3887→0.3557; disabled</td>
              </tr>
            </tbody>
          </table>
        </div>
      </div>

      <div className="section">
        <div className="section-label">MLX LoRA — Verdict Prediction</div>
        <div className="table-wrap">
          <table>
            <thead>
              <tr><th>Metric</th><th>Value</th></tr>
            </thead>
            <tbody>
              <tr><td>verdict_accuracy</td><td>0.695</td></tr>
              <tr><td>macro_F1</td><td>0.4632</td></tr>
              <tr><td>SUPPORTED F1</td><td>0.7024</td></tr>
              <tr><td>REFUTED F1</td><td>0.6872</td></tr>
              <tr><td>NOT ENOUGH INFO F1</td><td>0.0 (rarely predicted)</td></tr>
              <tr><td>citation_valid_rate</td><td>0.60</td></tr>
              <tr><td>throughput</td><td>53.7 tok/s (Apple Silicon)</td></tr>
            </tbody>
          </table>
        </div>
      </div>

      <div className="section">
        <div className="section-label">MLX LoRA — Explanation SFT (500 iters)</div>
        <p style={{ marginBottom: "12px", fontSize: "13px" }}>
          Base model: Qwen2.5-1.5B-Instruct-4bit · 256 training examples · Apple Silicon
        </p>
        <div className="table-wrap">
          <table>
            <thead>
              <tr>
                <th>Metric</th>
                <th>Base model</th>
                <th>300 iters</th>
                <th>500 iters</th>
              </tr>
            </thead>
            <tbody>
              <tr><td>format_correctness</td><td>0.0</td><td>0.20</td><td>0.28</td></tr>
              <tr className="row-highlight"><td>citation_presence</td><td>0.0</td><td>0.10</td><td>0.72</td></tr>
              <tr><td>decision_label_consistency</td><td>0.0</td><td>0.10</td><td>0.24</td></tr>
              <tr><td>avg explanation length (words)</td><td>0.0</td><td>34.2</td><td>33.2</td></tr>
            </tbody>
          </table>
        </div>
        <div className="notice notice-neutral" style={{ marginTop: "12px" }}>
          Generation bug (base_model key mismatch) fixed and adapter retrained.
          Citation presence 0.10→0.72. Partial format compliance — not production-grade.
          Small-sample training (256 examples, 500 iters).
        </div>
      </div>

      <div className="section">
        <div className="section-label">What Must Not Be Claimed</div>
        <div className="card">
          <ul style={{ paddingLeft: "18px", display: "flex", flexDirection: "column", gap: "8px", fontSize: "13px", color: "var(--text-muted)" }}>
            <li>ONNX is faster than native transformers on this Mac (it is not — 55 vs 62 ex/s)</li>
            <li>Robust verifier improved retrieved performance (both metrics regressed)</li>
            <li>Relevance gate improved macro-F1 (NEI FPR improved, macro-F1 regressed)</li>
            <li>Phi-3 QLoRA or DPO adapter was trained (CUDA not available)</li>
            <li>MLX explanation adapter achieves production-quality output (format correctness = 0.28)</li>
          </ul>
        </div>
      </div>
    </div>
  );
}
